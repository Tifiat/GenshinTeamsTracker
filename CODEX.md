...
- For UI work, especially in the Artifact Browser, avoid rebuilding whole widget lists when only selection state changes. Sync checked/selected state in place when list contents did not change.
...
- Persistent equipment Stage A/B is implemented in `hoyolab_export/account_equipment.py` and initialized through `hoyolab_export.artifact_db.init_db`: canonical `account_character_equipped_artifacts` references `artifacts.id` with one artifact owner max, canonical `account_character_equipped_weapons` references `account_weapon_observed_stacks.weapon_fingerprint` and validates assignments against `known_count`. No fake weapon instance ids are created. Existing `artifact_equipment` and weapon observed `equipped_character_id` metadata remain HoYoLAB provenance/seed inputs, not a second canonical state. AppShell now uses this state for weapon restore/assignment; Artifact Browser equip/apply is not wired yet.
- Persistent account equipment design and Stage B/B2/C1 behavior are documented in `docs/handoff/ACCOUNT_EQUIPMENT_STATE_DESIGN.md`. Artifact Browser equipment UX is documented in `docs/handoff/ARTIFACT_BROWSER_EQUIPMENT_UX.md`. Build presets remain reusable definitions; an explicit future `Надеть пресет` action should copy a preset into exactly one character's current equipped state, incomplete presets should clear missing target slots, and later manual equipment edits should not mutate the preset. HoYoLAB observation helpers exist but are not wired to live import; missing HoYoLAB equipment data means "no data", not "clear local equipment". AppShell restores current weapons and current artifacts from SQLite; artifact ids are converted into a runtime current-equipment snapshot for selected-details stats/set bonuses, not persisted as a fake saved build. Embedded Artifact Browser C1 is target-aware UI scaffolding only.
- AppShell performance status, 2026-05-26: use `GTT_PERF_LOG=1` or `python -m ui.app_shell_smoke --perf-log` for timing logs. The quick-pick marker bottleneck is fixed with a visible `character_id -> AssetIconLabel` registry and marker-only `set_selection_marker(...)` updates. Roster clicks now update markers immediately and schedule a debounced/coalesced right-panel refresh instead of running `RightPanelPrototypeWidget.set_model(...)` inside every click. AppShell filters use session-cached character/weapon asset lists, so normal filter clicks do not reread SQLite; roster/weapon card pixmaps use a shared scaled pixmap cache. Measured local smoke after this pass: character add handler about `0.7 ms` plus deferred right-panel flush about `56 ms`; 8 rapid character clicks about `2.6 ms` before one deferred flush; standard-character filter about `14 ms`, character filter clear about `111 ms`, weapon type filter about `18 ms`, and weapon filter clear about `95-96 ms`. Remaining work: right-panel `set_model(...)` still rebuilds full widgets and filter grid rebuilds still recreate visible card widgets, though from cached data/pixmaps.
...
- Isolated read-only TeamCard prototype exists as a pure view-model in `run_workspace/team_card_view_model.py` and a small QWidget in `ui/team_card_prototype.py`. Manual visual smoke launcher: `python -m ui.team_card_prototype_smoke` for real no-network `Тома` + build id 20, or `python -m ui.team_card_prototype_smoke --fake` for fake data. It consumes `TeamBuilderState` plus optional `CharacterDetailsData`, shows empty/filled four-slot teams, character/weapon/build labels, artifact summaries, statuses, and compact warnings. It is not wired into the legacy right panel and is not the final Run Workspace.
...
Future Artifact Browser equip-context design:

- There should be one shared Artifact Browser, not separate browser instances per slot.
- It should support account-wide browse mode and equip mode for exactly one operation target. If the right panel has a selected character, that character is the Artifact Browser operation target and initially syncs as the browser's single selected character so presets appear. If the user deselects it inside the browser, browser selection clears for preset browsing while the right-panel target remains as a secondary/background operation target for future free artifact clicks. If the right panel has no target, the browser may use exactly one selected character target. With 0 or 2+ browser-selected characters, equip mode is off and free artifact clicks do not equip.
- Current equipment is separate from presets. The browser should show a `Текущая сборка` top zone over persistent current equipment, not create a fake/temporary preset for it.
- Future current/preset zone should look like a compact preset row: plain `Текущий пресет` / `Текущая сборка` text with no dark button background, current set bonuses and main-stat badge, no edit/delete controls, and a real `Надеть пресет` action only when a saved preset is selected. Repeated click on the selected preset should deselect it and return to current equipment.
- Clicking a preset previews/selects it only. `Надеть пресет` is the explicit write action and copies the preset artifacts into exactly one character's current equipment through `account_equipment`. If the preset has missing slots, those target slots are cleared so the live equipment matches what the preset shows.
...
- Embedded Artifact Browser geometry is calibrated around a compact minimum-width landing: one `GRID_SIZE.width()` artifact cell, Assignment width 144px, target row about 94px, target rows using `MarqueeButton` with a reserved portrait/icon zone and marquee text only in the name area, fixed preset/current-equipment panel, and JSON import/clear controls that must not force the artifact viewport wider than one grid cell. The next geometry step is a pure divmod/remainder adaptive fit helper based on these source values; do not use candidate-width search or guessed card/gap constants. Remaining polish: clean JSON control placement/scaling and shift artifact grid overlay scrollbar visually right without consuming layout width.
