...
- `run_workspace/gcsim/`: isolated backend foundation for future local GCSIM engine lifecycle, including the transactional engine-store/update prototype and dev command `python -m run_workspace.gcsim.engine_update --release latest`; optional `--probe-runtime` runs Go through project-local `.go/` cache/bin paths, and explicit `--patch-backend git` applies ordered `.patch` stacks with `git apply`.
