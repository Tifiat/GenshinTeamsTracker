...
- Initial isolated engine lifecycle prototype exists in
  ...
  direct GCSIM config generation. Account character rows may contain localized
  display names such as Russian names, so a production GCSIM adapter must use
  stable IDs such as `character_id` plus explicit GCSIM mapping refs instead of
  searching English names in `account_characters.name`. The committed fixture
  remains synthetic because selected-team/current-build ownership and production
  ID-to-GCSIM mapping are still future work. A local smoke on 2026-06-05 used this
