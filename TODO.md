...
- Future AppShell tasks:
  ...
  - continue the production adapter: typed live-session ownership lives in `run_workspace/session.py`; immutable History Snapshot Bundle v2 now captures display details for every occupied slot, and RUN-page Save materializes declared visible assets inside grouped bundles without changing the live team state. Next, adapt Abyss/DPS Dummy snapshots into the shared right-panel view-model and add its read-only policy; right-command routing and durable GCSIM history attachment remain future work;
...
- Snapshot v2 now captures frozen display details for every occupied slot and
  production Save copies declared visible assets into the bundle. Next, adapt
  Abyss/DPS Dummy snapshots into a separate read-only instance of the same
  shared mode-specific Run panel; then add internal mode tabs, expandable Abyss
  period groups, compact visual rows, and newest-first ordering. The full contract is in
