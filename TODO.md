...
- Future AppShell tasks:
  ...
  - next right-panel code task: perform the global refactor toward `ui/right_panel/{common,live_run,history,pvp,settings}` plus `dock.py`/`header.py`, with imports and matching tests moved/updated in the same task. Detailed mode ownership is in `docs/handoff/APP_SHELL_WORKSPACE_PLAN.md`, `docs/handoff/PVP_UI_ROADMAP.md`, `docs/handoff/HISTORY_BROWSER.md`, and `docs/handoff/MAIN_UI_RIGHT_PANEL_INTEGRATION_AUDIT.md`;
