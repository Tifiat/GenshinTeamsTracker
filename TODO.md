...
- Future AppShell tasks:
  ...
  - right-panel add/select smoothness is now stable enough for manual UX: quick-pick uses fast UI state plus deferred hydration, team/slot widgets update in place, selected details use a stable skeleton/persistent bonus strip, and add-character no longer performs a visible intermediate minimal-details refresh before hydration;
