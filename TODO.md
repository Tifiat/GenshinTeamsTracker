...
- Future AppShell tasks:
  ...
  - keep PvP inside the AppShell `pvp` workspace/right-dock policy. Detailed PvP UI mode/stage direction lives in `docs/handoff/PVP_UI_ROADMAP.md`; Decks mode v0, Play/local match setup v0, and Draft board v0 are implemented. The next PvP code task is the backend/read-model `unified_pool` projection contract and sample fixture, followed by a `PvpDraftWorkspace` refactor to the readable unified visual pool; Team assignment remains after that;
...
- History is an AppShell left workspace owned by `ui/history_browser/`. It reads
  ...
  rows, and keeps the isolated empty History right-viewer separate from the live
  Run panel.
- Future Akasha-like rows, row selection, expanded export image/card,
  read-only History-specific right-panel details, and right-dock History command
  routing are contracted in `docs/handoff/HISTORY_BROWSER.md`.
