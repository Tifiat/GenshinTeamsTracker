...
- GCSIM Browser MVP layout:
  ...
  - current implemented batch action is `Run 3 chambers`: it maps the active Team 1/Team 2 tab to side 1/2, uses the same source identity and rotation editor text, runs C1/C2/C3 sequentially in the same Qt worker (no parallel GCSIM processes), and writes a compact batch report only into GCSIM Browser Results. Batch status is `passed`, `partial_failed`, or `failed`.
