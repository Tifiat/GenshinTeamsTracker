...
- Future AppShell tasks:
  ...
  - current smoothness priority: remove remaining right-panel hydration/layout twitch before hiding work behind startup loaders. Focus first on skip-unchanged/in-place updates for chamber/actions/bonus strip/selected details, lazy or prewarmed set-bonus tooltip descriptions, and stable selected-details height during minimal-to-hydrated character updates;
  - later AppShell performance work: bulk persistent equipment prewarm/negative cache for characters without equipment, then staged Artifact Browser cold-start init with persistent store/cache support;
