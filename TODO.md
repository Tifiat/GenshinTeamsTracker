...
- Future AppShell tasks:
  ...
  - continue the production adapter: next production-switch work starts with current in-memory run/session behavior for the right dock (timer editing, reset, elapsed-time and factual-DPS results) without reviving legacy widget ownership. Immutable saved snapshots, snapshot persistence, and History opening come later, after working timer/DPS/GCSIM result data exists. Richer `CharacterDetailsData` preparation can continue incrementally where the selected-details UI still needs it;
