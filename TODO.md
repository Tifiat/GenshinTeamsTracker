...
- Separate audit prompt exists at `ABYSS_ENEMY_DATA_AUDIT_TASK.md`. Use it for a neighboring/new chat to research public Abyss enemy data sources and create `ABYSS_ENEMY_DATA.md` with source keys, HP/resist/wave/mechanics coverage, risks, and MVP source recommendation.
...
- Current explicit refresh path: `python -m hoyolab_export.hoyowiki_catalog_refresh`. It refreshes `data/cache/hoyowiki/character_stats_catalog.json` and `data/cache/hoyowiki/weapon_stats_catalog.json` from HoYoWiki lists plus detail pages, uses `en-us` by default, skips valid cached entries in missing-only mode, and supports `--force` after parser/source changes.
...
- Minimal `CharacterStatSnapshot` foundation exists for ordinary matched-ready characters/weapons. It is read-only/backend-only and partial: it preserves character base HP/ATK/DEF, ascension bonus separately, weapon base ATK/secondary stat, optional artifact summary, and warnings. It does not compute final totals, does not apply weapon passive text, set bonus formulas, conditional bonuses, or resonances. Traveler remains `special_deferred`.
- Account character source shape is documented in `CODEX.md`: current account data exposes character level/constellation/etc. but no reliable character promote/ascension phase. MVP character row policy therefore assumes `after` ascension for levels 20/40/50/60/70/80 and marks `character_ascension_phase_assumed`; level 90 uses final before/single value where present. For rare 95/100 levels, use exact catalog rows if present, otherwise fall back to level 90 with `level_95_100_fallback_to_90`; do not interpolate.
...
- TeamCard / CharacterDetails backend data adapter exists in `hoyolab_export/team_card_data.py`. It accepts explicit selected account character + selected account weapon + prepared build snapshot, or an outer read-only build-id loader that converts the selected Artifact Browser preset to `ArtifactBuildSnapshot`. `CharacterStatSnapshot` remains DB/UI-free. The adapter returns serializable-friendly `CharacterDetailsData` with selected character/weapon/build provenance, the partial stat snapshot, warnings, and GCSIM-readiness notes.
- Real no-network `CharacterDetailsData` smoke exists: `python -m hoyolab_export.team_card_data_smoke --character-name "Тома" --build-id 20`. It reads allowlisted account JSON, local HoYoWiki stats caches, and `data/artifacts.db` read-only for the selected build id. The smoke validates selected character + equipped weapon + build id -> prepared details data with artifact contribution present, while still not computing final totals or applying formulas.
...
- Stat normalization / GCSIM stat-key mapping handoff exists at `STAT_NORMALIZATION.md`. Current backend follow-up while UI design is being worked out: add a pure stat-normalization layer that maps project `property_type` values to normalized keys/GCSIM keys, converts artifact percent-point values like `46.6` to decimal ratios like `0.466`, preserves source/provenance, and keeps final totals/GCSIM config generation out of scope.
- Future follow-up: if a reliable character ascension phase/max-level source appears, wire it into `SnapshotAccountCharacter.promote_level` and remove or narrow the MVP assumption warning. Revisit rare 95/100 handling only when exact HoYoWiki/source data are confirmed beyond the current fallback-to-90 policy.
- Next Run Workspace/UI step after the visual direction is settled: refine the TeamCard data/visual contract as needed, then plan or build the first Run Workspace / TeamBuilder shell around the shared TeamCard concept before replacing the legacy right panel.
...
- Future Standard 5-star filter: asset exists at `assets/filters/standard.png`; desired tri-state is show all / only Standard 5-star / exclude Standard 5-star with red crossed visual state. Earlier audit found HoYoWiki `entry/2952` currently returns Acquaint Fate rather than a structured standard 5-star list, so use another confirmed source or a manual seed catalog later.
- Future storage audit/refactor: once catalog/account/cache usage is clearer, decide which generated JSON/cache files should stay as small rebuildable source caches or seeds and which should be normalized into SQLite/catalog DB tables. Prefer DB storage only for runtime-critical data that is frequently joined, mapped, filtered, queried, reported, or used by stat calculator/UI. Likely candidates include HoYoWiki character stats, weapon stats, character traits, character region catalog, mapping reports / alias override tables, and other large generated account/catalog JSON files after usage audit. A two-layer model is acceptable: raw/source JSON cache for fetched HoYoWiki/HoYoLAB data plus normalized DB tables for lookup, mappings, aliases, reports, UI, and stat calculation.
...
- Detailed GCSIM research handoff lives in `GCSIM.md`; read it before implementing any engine download/runner/config work.
- Stat/GCSIM `add stats` key mapping handoff lives in `STAT_NORMALIZATION.md`; implement that normalization layer before final stat totals or GCSIM config generation.
