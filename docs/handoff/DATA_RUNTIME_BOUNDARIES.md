...
| --- | --- |
| `data/gcsim/optimizer-go-runs/` | `optimizer_run_retention.py`, invoked by the shared Selected/All Sets session finally path;50 completed runs/256MiB ordinary budget; current/OS-leased/manual-pinned runs protected, newest oversized result reported explicitly |
...
research residue. Shared Selected/All Sets runs call lease-aware retention from
their common `finally` path. Successful engine updates prune generated engine
