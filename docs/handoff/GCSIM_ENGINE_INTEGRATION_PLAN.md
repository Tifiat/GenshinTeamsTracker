...
Account-backed backend/dev prepared config bridge now exists in `run_workspace/gcsim/account_prepared_config.py`. CLI: `python -m run_workspace.gcsim.account_prepared_config --format text|json`; optional cached-Abyss smoke: add `--run-abyss-smoke`. It reads real `data/artifacts.db` account rows for Chasca/Ororon/Furina/Bennett, consumes stored ready `account_characters.gcsim_character_key` and `account_weapon_observed_stacks.gcsim_weapon_key`, and never uses localized `name` fields as GCSIM identity. Current-equipped weapon rows are used when present; otherwise a deterministic ready observed weapon stack of the matching weapon type is selected and reported with `dev_weapon_candidate_not_account_truth`. Artifacts are still synthetic/dev in this bridge and always reported with `synthetic_dev_artifact_stats_not_account_truth`; current artifact rows may be counted for diagnostics, but they are not consumed as account-truth `add stats` until artifact-set mapping and selected/current build ownership are confirmed. The adapter writes no partial config when any character block is not ready. It does not query UI, persist right-panel state, mutate DB/schema, run network refreshes, rebuild GCSIM, or change the patch stack. Local no-network diagnostic on 2026-06-06 generated an account-backed Chasca/Ororon/Furina/Bennett full config and passed cached `2026-02-16` F12 C1 S1 through the active artifact as a backend compatibility smoke only; do not treat the smoke summary as DPS correctness. Production selected-team/current-build ownership, artifact-set mapping, and final UI wiring remain future work.

## 8. Talent Levels

Current GCSIM v2.42.2 validates config talent levels as parser/base values in the inclusive range `1..10` (`pkg/core/player/character/character.go`). Account/HoYoLAB observed talent rows may include constellation-boosted displayed levels above 10. The backend/dev account bridge caps those values to 10 only for parser compatibility and reports `talent_level_capped_to_gcsim_parser_range_not_account_truth`; this is not final talent truth and should not be used for DPS correctness claims.

A later implementation task must confirm whether the production adapter should derive base talent levels by subtracting constellation bonuses, use another GCSIM-supported representation, or change the GTT-GCSIM patch contract. It must also keep a deterministic normal/skill/burst ordering helper for account talent rows.
