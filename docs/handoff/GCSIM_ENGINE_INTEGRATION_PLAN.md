...
- A real local artifact smoke also succeeded on 2026-06-04 through `run_smoke`, using a tiny upstream-doc-style config with `iteration=1` and `duration=2`. The parsed summary reported non-empty DPS, duration, total damage, sim version, warnings, and failed actions from the generated JSON.
- Next real-engine tasks should add shipped-engine fallback contract support, config/key-mapping foundations, stronger simulation smoke configs, and then the first sequential-wave research patch. Do not wire this into UI until engine preparation, config generation, and result boundaries are validated.
