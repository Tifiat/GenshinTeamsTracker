...
- Dev coverage checker `python -m run_workspace.gcsim.abyss_enemy_type_mapping_report --enemy-type-map path --gcsim-enemy-registry-source path --cache-file path --format text` verifies cached Abyss source-data against optional overrides plus the GCSIM enemy type registry without network fetches, cache mutation, GCSIM runs, or UI imports. It reports total rows, resolution counts by method (`manual_mapping`, `exact_normalized_name`, `compatible_base_name`, `manual_alias`), missing/ambiguous mappings, HP-present/type-missing rows, type-present/HP-missing rows, JSON resolved rows with selected type/method, and unresolved rows with all available identities. Use it for both normal Nanoka-backed caches and forced Fandom fallback/Fandom-only caches. Full production enemy type mapping coverage remains future work.
