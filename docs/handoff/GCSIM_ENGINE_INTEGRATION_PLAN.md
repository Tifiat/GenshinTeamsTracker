...
- Current check status is source-layout only. The manifest/report deliberately records `runtime_ready=false` and `runtime_check_status=not_run_no_build_step_yet` because no Go build or executable smoke check is implemented yet.
- Tests in `tests/test_gcsim_engine_update.py` pin fake official-source acquisition, download failure, corrupt archive failure, layout-smoke failure, manifest metadata, and old-active preservation.
