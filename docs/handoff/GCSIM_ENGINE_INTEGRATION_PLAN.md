...
- Next real-engine tasks should add shipped-engine fallback contract support, config/key-mapping foundations, stronger simulation smoke configs, and app-side scenario payload generation from typed run/enemy data. Do not wire this into UI until engine preparation, config generation, and result boundaries are validated.
