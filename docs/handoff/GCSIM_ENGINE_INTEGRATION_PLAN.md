...
- Map project/account character, weapon, artifact set, and enemy identifiers to GCSIM keys using stable IDs or generated mapping data. For account characters, `account_characters.name` is localized display text; use `character_id` plus explicit mapping refs rather than English-name searches in the account table.
