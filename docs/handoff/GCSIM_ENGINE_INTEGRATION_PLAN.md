...
- Current `-gtt-info` for a built patched artifact reports `gtt_engine=true`, `gtt_patch_version=gtt-wave-prototype-v1`, `capabilities=["gtt_engine_marker","gtt_wave_scheduler_prototype"]`, `sequential_waves=true`, `wave_scheduler_stage="prototype"`, and the upstream version when available. This is a prototype capability signal, not a final Abyss-correctness claim.
...
- A real local validation command succeeded on 2026-06-04:
  ...
  It built official upstream `v2.42.2`, activated engine `gcsim-v2.42.2-20260604172036`, produced `build/gtt-gcsim.exe` with sha256 `c5692e11a22062525c2aa16ff4499f9f90f80538ac2da318564ea30d8d67b449`, and `-gtt-info` returned the marker plus `gtt_wave_scheduler_prototype` with `sequential_waves=true`.
...
- Next real-engine tasks should add shipped-engine fallback contract support, config/key-mapping foundations, stronger simulation smoke configs, then replace the prototype duplicate-first-target directive with a real GTT scenario/wave payload. Do not wire this into UI until engine preparation, config generation, and result boundaries are validated.
